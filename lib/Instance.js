...
let Model, Utils, InstanceSet, $request, $guid
...
class Instance {
  ...
  constructor(model, values) {
    ...
    if(_.isNil(values)) {
      values = this.$model().defaults({strict: true, deep: true, plain: true})
    }

    // Añadimos todos los atributos y asociaciones especificados en el modelo
    // para que al llamar a "set" mas adelante este cree propiedades para cada
    // uno de estos
    values     = _.defaults({}, values, _.mapValues(model.attributes, ()=> undefined))
    values     = _.defaults({}, values, _.mapValues(model.associations, (assoc, alias)=> assoc.kind.endsWith('Many') ? [] : undefined))
  ...
  $get(path, options) {
    options = _.isPlainObject(path) ? path : options || {}
    options = _.defaults(options, { strict: false, deep: true, oids: false })
    path = _.isString(path) ? path : undefined
    if(path != undefined) {
      if(/[.\[\]]/.test(path))
        return Utils.$getPropertyDeep(this, path)

      if(this._dataValues[path] !== undefined)
        return this._dataValues[path]
      else
        return this._associations[path]
    } else {
  ...
  $set(attr, val, options) {
    if(!_.isString(attr)) {
      if(_.isInteger(attr) || _.isPlainObject(attr) || attr instanceof Instance) {
        options = val
        val = attr instanceof Instance ? attr.$get() : attr
        attr = undefined
      }
    ...
    _.forIn(val, (value, key)=> {
      // Ignorar atributos cuyo nombre no inicie con letra alfabetica
      if(!/^[a-zA-Z]/.test(attr)) return
      ...
      // Solo asignar valor indefinido si se esta estableciendo un atributo especifico
      ...
      if(assoc) {
        ...
        if(!checkTypes(value)) throw new Error(
          'Valid value types for associations include null, integer, Instance, Object, ' +
          'InstanceSet, or Array with the same types'
        )

        let assocModel = this.$registry[assoc.model]
        if(!assocModel) throw new Error(`Model "${assoc.model}" is not defined`)
        if(assoc.kind.endsWith('Many')) {
          if(!_.isArray(value) && !_.isNil(value)) throw new Error('The value for a x:M association should be an array')
  ...
  $raw(options) {
    ...
    return _.cloneDeep(
      _.omitBy(
        ...
        ), (value)=> _.isUndefined(value) || _.isArray(value) && _.isEmpty(value)
      )
    )
  }
  ...
  async $save() {
    ...
    return await $request(endpoint, method, requestData)
  }

  async $delete() {
    return await $request(this.$model().endpoint + '/' + this[this.$model().pk], 'DELETE')
  }

  $reload() {
    return $request(this.$model().endpoint + '/' + this[this.$model().pk], 'GET')
  }
  ...
  $modified(previousDataValues) {
    ...
    for (let prop in this.$attributes()) {
      // Si es funcion o si su nombre no inicia con caracter alfabetico
      if (_.isFunction(this[prop]) || !prop.match(/^[a-zA-Z]/))
        continue
