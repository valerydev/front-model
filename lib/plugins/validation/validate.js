...
module.exports = function(models) {
  ...
  Instance.prototype.validate = function(path) {
    if(!_.isEmpty(path)){
      let instance = eval('this.' + path
          .replace(/(?:\.)(\d+)(\.?)/g, (match, $1, $2)=> '[' + $1 + ']' + $2 || '')
          .replace(/\[(\d+)\]/g, (match,$1)=> '.get('+$1+')'))
    ...
    for(let attrName in attributes) {
      ...
      let attrVal = (this[attrName] != undefined && this[attrName] != null) ? this[attrName] : ''
    ...
    for( let alias in associations ) {
      ...
      if (this[alias]) {
        let items = assoc.kind.endsWith('Many') ? this[alias].toArray() : [this[alias]]
        for(let item of this[alias].toArray()) {
          let assocErrors = item.validate()
          errors[alias] = assocErrors && assoc.kind.endsWith('Many') ?
    ...
    if(!_.isEmpty(errors)) {
      return traverse(errors).reduce(function(memo, x) {
        if (this.isLeaf){
          memo[this.path.join('.')
              .replace(/\.\d+\.(msg|kind)$/,'')
