    ...
    _.each(val, (value, key)=> {
      ...
      if(assoc) {
        ...
        if(assoc.type.endsWith('Many')) {
          ...
          let createSet = (values)=> {
            let valueSet = new Set.CollectionsSet(values)
            valueSet.contentEquals = (function(self) {
              return function(a,b) {
                let equals = (_.isInteger(a)?a:a.id) === (_.isInteger(b)?b:b.id) || (_.get(a, '_oid') != undefined && _.get(a, '_oid') === _.get(b, '_oid'))
                if(equals) {
                  if(a instanceof Instance && (b instanceof Instance || _.isPlainObject(b)))
                    a.set(_.omit(b, assoc.pk, '_oid'))
                  else { 
                    if(_.isPlainObject(b)) { 
                      let newInstance = new Instance(assocModel, _.omit(b, assoc.through))
          ...
          let valueSet = createSet(value)
          let currAssoc = this._associations[key]
          if( currAssoc instanceof Set.CollectionsSet) 
            valueSet = currAssoc.intersection(valueSet).addEach(valueSet)
          this._associations[key] = valueSet
        }
        else {
