...
class Instance {
  ...
  constructor(model, values) {
    ...
    this._dataValues = {}
    this._associations = {}
    this._previousDataValues  = this.$attributes(values)
  ...
  set(key, val, options) {
    if(!_.isString(key)) {
      if(_.isInteger(key) || _.isPlainObject(key)) {
        options = val
        val = key
      }
    ...
    _.each(val, (value, key)=> {
      ...
      if(assoc) {
        ...
        if(assoc.type.endsWith('Many')) {
          ...
          _.difference( manyValues,
            _.compact(_.map( manyValues, (values)=> {
        ...
        options.deep ?
          _.mapValues( this.$associations(), (it)=> {
            if(it instanceof Instance) {
              ...
              return _.map(it, (it)=> it instanceof Instance ? it.$raw(options) : it)
            } else {
              return it
            }
          }) : {}
  ...
  isDirty() {
    return !_.isEqual(this.get({deep: true}), this._previousDataValues)
  }
  ...
  modified() {
    ...
    for (var alias in associations) {
      ...
      if (associations[alias].type.endsWith('Many')){
        var orig    = this._previousDataValues[alias]
        let modified = _.union(
          // Asociaciones por Eliminar
          _.differenceBy(orig, current, assocPk).map((it)=> _.pick(it, assocPk)),
          // Asociaciones por crear
          _.filter(current, (it)=> !_.isInteger(it) && !it[assocPk]).map((it)=> it.get({deep: true})),
          // Asociaciones con modificaciones o por asociar/deasociar
          _.compact(
            _.intersectionWith(current, orig , (c,o)=> _.isInteger(c) || c[assocPk] == o[assocPk]).map((it)=> {
