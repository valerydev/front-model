...
class Instance {
  ...
  constructor(model, values) {
    ...
    this._previousDataValues = this.get({deep: true})
  }
    ...
    for (var alias in associations) {
      ...
      if (associations[alias].type.endsWith('Many')){
        ...
        let modified = _.union(
          ...
          _.differenceWith(prev, current, (p,c)=> (p[assocPk] == undefined) && (p[assocPk] == (_.isInteger(c)?c:c[assocPk])) ).map((it)=> _.pick(it, assocPk)),
          // Asociaciones nuevas
          _.differenceWith(current, prev, (c,p)=> (p[assocPk] == undefined) && (p[assocPk] == (_.isInteger(c)?c:c[assocPk])) ).map((it)=> _.isInteger(it)?it:it.get({deep:true})),
