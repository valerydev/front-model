...
class InstanceSet extends Set.CollectionsSet {
  ...
  constructor(values, assoc) {
    ...
    if(!_.isPlainObject(assoc)
        || _.isUndefined(registry[assoc.model])
        || !/^(BelongsTo|BelongsToMany|HasMany|HasOne)$/i.test(assoc.type)
        || assoc.type === 'BelongsToMany' && _.isEmpty(assoc.through))
  ...
  add(values) {
    ...
    let processThrough = function (assoc, instance, values) {
      if(assoc.through) {
        ...
        let throughModel = registry[assoc.through]
    ...
    if(!_.isUndefined(a) && !_.isUndefined(b)) {
      if(a instanceof Instance && (b instanceof Instance || _.isPlainObject(b))) {
        a.set(_.omit(b, assoc.pk, '_oid', assoc.through))
